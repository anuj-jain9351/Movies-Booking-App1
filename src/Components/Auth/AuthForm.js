...
const AuthForm = ({onSubmit,isAdmin}) => {
  ...
  return (<Dialog PaperProps={{style:{ borderRadius:20}}} open={true} >
   ...
   <form onSubmit={handleSubmit}>
    <Box padding={4} display={'flex'} justifyContent={'center'} flexDirection="column" width={400} height={350} margin="auto" alignItems={"center"}>
    { !isAdmin && isSignup && ( <> {" "} <FormLabel variant="standard" sx={labelStyle}>Name</FormLabel>
    <TextField  value={inputs.name} onChange={handleChange} margin="normal" variant="standard" type={"text"} name="name"/></>)}
        <FormLabel variant="standard" sx={labelStyle}>Email</FormLabel>
